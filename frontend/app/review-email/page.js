...
export default function EmailReviewPage() {
    ...
    // Add this button next to the "Check Inbox" button
    <button
    ...
    const handleReview = async (action) => {
        ...
        if (res.ok) {
            const data = await res.json();
            showToast(`Submission ${action}ed successfully!`, action === 'accept' ? 'success' : 'error');
            
            // Refresh submissions
            fetchSubmissions();
            setReviewNotes('');
            
            // If accepted, show the submission ID
    ...
    return (
        ...
        <div className="bg-white border-b border-slate-200 px-8 py-6">
            ...
            <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Review and process email submissions from the Gmail inbox.</p>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-8 py-6">
            ...
            <div className="bg-white rounded-xl p-4 border border-slate-200">
                <p className="text-2xl font-bold text-emerald-600">{submissions.filter(s => s.status === 'accepted').length}</p>
